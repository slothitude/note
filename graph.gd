...
func _execute_node(exec_node: GraphNode) -> void:
	...
	for conn in connections:
		if conn.to == exec_node.name and conn.to_port == 0:
			var source := graph_edit.get_node_or_null(NodePath(conn.from))
	...
	var output := []
	var error := OS.execute("cmd", ["/C", input_text], output)

	var stdout := "".join(output) if output.size() > 0 else ""
	var stderr := "Error: exit code %d" % error if error != 0 else ""

	for conn in connections:
		if conn.from == exec_node.name and conn.from_port == 0:
			var target := graph_edit.get_node_or_null(NodePath(conn.to))
			if target and target.has_method("set_text"):
				target.set_text(stdout)

	if stderr != "":
		for conn in connections:
			if conn.from == exec_node.name and conn.from_port == 1:
				var target := graph_edit.get_node_or_null(NodePath(conn.to))
				if target and target.has_method("set_text"):
					target.set_text(stderr)
