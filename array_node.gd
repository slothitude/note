...
func _ready() -> void:
	...
	if mode_option:
		...
		mode_option.add_item("Index")
		mode_option.add_item("Length")
		mode_option.add_item("Push")
		mode_option.add_item("Sort")
	AssemblerScript.configure_slots(self, "array")
	_add_control_ports()
...
func _evaluate() -> void:
	...
	var items := input_text.split("\n", false)
	for i in range(items.size()):
		items[i] = str(items[i]).strip_edges()

	match mode:
		0:  # Join: combine with delimiter
			var delim: String = p if p != "" else ","
			result_text = delim.join(items)
		1:  # Split: split input by delimiter into newline-separated (identity if already newline-sep)
			var delim: String = p if p != "" else ","
			if delim == "\n":
				result_text = input_text
			else:
				var parts := input_text.split(delim, false)
				var cleaned: PackedStringArray = []
				for part in parts:
					cleaned.append(str(part).strip_edges())
				result_text = "\n".join(cleaned)
		2:  # Index: get element at position
			var idx := p.to_int() if p.is_valid_int() else -1
			if idx >= 0 and idx < items.size():
				result_text = items[idx]
			else:
				result_text = ""
		3:  # Length
			result_text = str(items.size())
		4:  # Push: append value
			if p != "":
				items.append(p)
			result_text = "\n".join(items)
...
func get_gal_props(nd: Dictionary) -> Dictionary:
	var mode_map: Dictionary = {0: "JOIN", 1: "SPLIT", 2: "INDEX", 3: "LENGTH", 4: "PUSH", 5: "SORT"}
	var props: Dictionary = {"mode": mode_map.get(nd.get("mode", 0), "JOIN")}
